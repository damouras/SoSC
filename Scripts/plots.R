...
aprogs %>%  group_by(UNIVERSITY) %>% slice(1) %>%
  ...
  ggtitle("Universities with Stats Hons/Spec Programs by Size") + labs(y="TOTAL UNIVERSITY ENROLMENT (2016)") 
...
aprogs_dis %>% group_by(UNIVERSITY,Discipline) %>% summarize(n_Courses = sum(Credits)*2) %>% 
  ...
  ggplot(aes(x=Discipline,y=n_Courses, fill=Discipline)) + geom_boxplot() + labs(y="# of COURSES") 
...
aprogs_dis_lev %>% group_by(Discipline, Level) %>% summarise(Credits=sum(Credits)*2/24) %>% 
  ...
  ggtitle("Average # of Courses per Course Discipline and Level") + labs(y="# of COURSES") 
...
aprogs_cat %>% group_by(UNIVERSITY,Category) %>% summarize(n_Courses = sum(Credits)*2) %>% 
  ...
  ggtitle("Boxplots of # of Courses per Topic Group") + labs(y="# of COURSES", x="TOPIC GROUP")
